...
# Wikipedia article
# https://github.com/zembrodt/pymeteor/blob/master/pymeteor/pymeteor.py
...
def meteor(reference, candidate, print_details=False, detailed_result=False):
    if print_details:
    ...
    score_meteor = _calculate_meteor(f_mean, penalty_val)
    ...
    if not detailed_result:
        ...
        M = dict(
            ...
            # chunks=penalty[1],
            # mappings=penalty[2],
            # fragmentation=(penalty[1]/float(penalty[2])),
            precision=P,
            recall=R,
            f_mean=f_mean
        )
    ...
    if print_details:
        ...
        print('Score: {:.4f} = Fmean: {:.4f} * (1 - Penalty: {:.4f})'.format(score_meteor, f_mean, penalty_val))
