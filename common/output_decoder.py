# import warnings
# from algorithms.symbolicTransformer.src.tools.helper import tile
...
def beam_search(model, test_data_src: List[List[str]], beam_size: int, max_decoding_time_step: int): # -> List[List[Hypothesis]]:
    ...
    with torch.no_grad():
        for src_sent in tqdm(test_data_src, desc='Decoding', file=sys.stdout):
            example_hyps = model_beam_search(model, src_sent, beam_size=beam_size, max_decoding_time_step=max_decoding_time_step)
...
def model_beam_search(model, src_sent: List[str], beam_size: int = 5, max_decoding_time_step: int = 70):  # -> List[Hypothesis]:
    ...
    src_encodings, dec_init_vec = model.encode(src_sents_var, [len(src_sent)])
    ...
    while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
        ...
        (h_t, cell_t), att_t, _ = model.step(x, h_tm1, exp_src_encodings, exp_src_encodings_att_linear, enc_masks=None)
...
# normally deprecated
# BEAM SEARCH ----------------------------------------------------------------------------
# source : https://github.com/OpenNMT/OpenNMT-py/blob/master/onmt/translate/beam_search.py
#
